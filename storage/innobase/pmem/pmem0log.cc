	...
	for (i = 0; i < n; i++) {
		...
		pl->pmem_tx_log_size += sizeof(PMEM_TX_LOG_BLOCK);
			...
			POBJ_ALLOC(pop,
					...
					sizeof(TOID(PMEM_DPT_ENTRY_REF)) * MAX_DIRTY_PAGES_PER_TX,
					NULL,
					NULL);
			uint64_t i_temp;
		...
		pl->pmem_tx_log_size += ( sizeof(TOID(PMEM_DPT_ENTRY_REF)) + sizeof(PMEM_DPT_ENTRY_REF)) * MAX_DIRTY_PAGES_PER_TX;
